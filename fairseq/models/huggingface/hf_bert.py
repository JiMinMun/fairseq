...
# class HuggingFaceAutoLanguageModel(FairseqLanguageModel):
@register_model('hf_auto')
class HuggingFaceAutoLanguageModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--load-hf-auto-from', type=str, default='',
                            help='load huggingface pretrained auto from path')

        parser.add_argument('--load-hf-auto-config-only', action='store_true',
    ...
    def build_model(cls, args, task):
        ...
        assert args.load_hf_auto_from != ''
        encoder = HuggingFaceAutoEncoder(args, task.dictionary)
    ...
    def register_classification_head(self, name, num_classes=None, inner_dim=None, **kwargs):
        ...
        self.classification_heads[name] = HuggingFaceAutoClassificationHead(
...
class HuggingFaceAutoClassificationHead(nn.Module):
...
class HuggingFaceAutoEncoder(FairseqEncoder):
    def __init__(self, args, dictionary):
        ...
        load_hf_auto_from = getattr(args, 'load_hf_auto_from', '')
        assert load_hf_auto_from != ''
        model_path = load_hf_auto_from

        config = AutoConfig.from_pretrained(model_path)

        #logging.info("args: {}".format(args))
        if getattr(args, 'load_hf_auto_config_only', False) is True:
            logger.info(
                "now we will init the hf_auto model from config without the weights,"
                " since we will restore the weights later")
            self.model = AutoModel.from_pretrained(model_path)
        else:
            logger.info("now we will init the hf_auto model from {} with all the weights".format(model_path))
            self.model = AutoModel.from_pretrained(model_path)
...
@register_model_architecture('hf_auto', 'hf_auto_base')
